...
## STEP 3 — Google Sign-In Fix Karo (Zaroori)

Google login ab tabhi kaam karega jab tumhare Firebase project me Android app properly register ho:

1. **Firebase Console** → apna project (`bharatmitra-ai`) → ⚙️ Project Settings → **General** tab
2. Neeche "Your apps" me **Add app → Android** dabao
3. Android package name me exactly ye daalo: `com.bharatmitra.ai`
4. **Debug signing certificate SHA-1** field me ye fixed value paste karo (ye maine tumhare liye generate ki hai, taaki har build me same rahe):
   ```
   DC:3A:6C:2E:E8:CA:08:0C:98:2F:3C:D2:AA:02:40:83:22:AF:70:C0
   ```
5. **Register app** dabao → **google-services.json download karo**
6. Us file ko is zip ke **root folder** me rakh do (`package.json` ke saath, same level pe) — naam bilkul `google-services.json` hi rakhna
7. Firebase Console me hi **Authentication → Sign-in method → Google** ko **Enable** karo (agar pehle se nahi hai)

Bas — CodeMagic build khud `google-services.json` ko sahi jagah copy kar dega aur fixed `debug.keystore` use karega taaki SHA-1 hamesha match kare.
